...
from app.services.pipeline_service import list_jobs
...
def update_preferences():
    ...
    if section == "profile":
        ...
        requested_start_page = request.form.get("start_page", "home")
        if (
            not current_app.config.get("POST_UPLOAD_PAGES_ENABLED", False)
            and requested_start_page == "history"
        ):
            requested_start_page = "home"
            flash("History is currently unavailable. Start page kept as Home.", "info")
        existing_settings["start_page"] = requested_start_page
