...
PENDING_OTP_EMAIL_KEY = "pending_otp_email"
PENDING_OTP_PURPOSE_KEY = "pending_otp_purpose"
...
def _set_pending_otp(email: str, purpose: str) -> None:
    session[PENDING_OTP_EMAIL_KEY] = email
    session[PENDING_OTP_PURPOSE_KEY] = purpose
    session.modified = True


def _clear_pending_otp() -> None:
    session.pop(PENDING_OTP_EMAIL_KEY, None)
    session.pop(PENDING_OTP_PURPOSE_KEY, None)
    session.modified = True
    ...
    if request.method == "POST":
        ...
        if existing_user:
            ...
            if has_recent_otp(existing_user.id, cooldown):
                ...
                delivered, msg = _issue_otp(existing_user)
                flash(msg, "success" if delivered else "info")

            _set_pending_otp(email=email, purpose=OTP_PURPOSE_VERIFY)
...
def verify_otp_page():
    ...
    requested_email = _normalize_email(request.values.get("email", ""))
    pending_email = _normalize_email(session.get(PENDING_OTP_EMAIL_KEY, ""))
    pending_purpose = session.get(PENDING_OTP_PURPOSE_KEY, "")
    email = requested_email or pending_email
    ...
    if request.method == "POST":
        ...
        login_challenge = (
            pending_purpose == OTP_PURPOSE_LOGIN and pending_email == email
        )
        if user.is_verified and not login_challenge:
            flash("Please login first to receive a fresh OTP.", "warning")
            return redirect(url_for("auth.login"))
...
def resend_otp():
    requested_email = _normalize_email(request.form.get("email", ""))
    pending_email = _normalize_email(session.get(PENDING_OTP_EMAIL_KEY, ""))
    pending_purpose = session.get(PENDING_OTP_PURPOSE_KEY, "")
    email = requested_email or pending_email
    ...
    if user.is_verified:
        if pending_purpose != OTP_PURPOSE_LOGIN or pending_email != email:
            flash("Please login first to receive a fresh OTP.", "warning")
            return redirect(url_for("auth.login"))
        delivered, msg = _issue_otp(user)
        _set_pending_otp(email=email, purpose=OTP_PURPOSE_LOGIN)
        flash("Login OTP generated.", "success")
        flash(msg, "success" if delivered else "info")
        return redirect(url_for("auth.verify_otp_page", email=email))
...
def login():
    ...
    if request.method == "POST":
        ...
        delivered, msg = _issue_otp(user)
        flash("Enter the OTP sent to your email to complete login.", "info")
        flash(msg, "success" if delivered else "info")
        return redirect(url_for("auth.verify_otp_page", email=email))
