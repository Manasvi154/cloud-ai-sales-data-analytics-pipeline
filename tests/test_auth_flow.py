...
def test_verified_user_still_requires_otp_on_login(monkeypatch):
    app, _ = _build_test_app(monkeypatch)
    client = app.test_client()

    client.post(
        "/auth/register",
        data={"email": "otp-every-login@example.com", "password": "StrongPass123"},
        follow_redirects=True,
    )
    client.post(
        "/auth/verify",
        data={"email": "otp-every-login@example.com", "otp": "123456"},
        follow_redirects=True,
    )
    client.post("/auth/logout", follow_redirects=True)
    ...
    assert "/auth/verify" in login_response.headers.get("Location", "")
    ...
    assert b"OTP verified. Logged in successfully." in verify_response.data
